...
public interface JsOrdersRepository extends JpaRepository<RegionalSalesDTO, Integer> {
     ...
     * @return 区域销售额
     */
    @Query(nativeQuery = true, value = "SELECT REPLACE(os.province,'省','') AS 'name', SUM(os.amount) / 10000 AS 'value',CONCAT(SUM( os.amount ) / ( SELECT SUM( amount ) from JS_Orders WHERE create_time >= :startCreateTime AND create_time <= :endCreateTime ) * 100, \"%\") AS 'percent'" +
